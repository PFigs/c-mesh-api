...
// Limitation comes from the maximum frame size in DualMCU API
#define MAXIMUM_DATA_ITEM_PAYLOAD_SIZE 251
#define MAXIMUM_DATA_ITEM_LIST_LENGTH 80
...
typedef struct __attribute__((__packed__))
{
    ...
    uint8_t payload[MAXIMUM_DATA_ITEM_PAYLOAD_SIZE];
...
} msap_config_data_item_list_req_pl_t;

typedef struct __attribute__((__packed__))
{
    uint8_t result;
    uint8_t max_optional_item_payload_length;
    uint8_t max_num_of_optional_items;
    uint8_t number_of_items;
    struct __attribute__((__packed__)) {
        uint16_t endpoint;
        uint8_t payload_length;
    } items[MAXIMUM_DATA_ITEM_LIST_LENGTH];
} msap_config_data_item_list_conf_pl_t;
 ...
 * \param    payload
 *           Pointer to the read data item list.
 * \return   Negative value upon internal error, otherwise result code returned
 *           by the DualMCU API
 */
int msap_config_data_item_list_request(const uint8_t request_type,
                                       msap_config_data_item_list_conf_pl_t *const payload);
