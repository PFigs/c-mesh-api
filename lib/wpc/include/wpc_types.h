...
typedef struct __attribute__((__packed__))
{
    ...
    union
    {
        ...
        msap_config_data_item_list_req_pl_t msap_config_data_item_list_request_payload;
        // Indication
        ...
        msap_config_data_item_list_conf_pl_t msap_config_data_item_list_confirm_payload;
        // Response
        sap_resp_pl_t sap_response_payload;
    } payload;
} wpc_frame_t;

// Make sure each member of the payload union has a size representable by 8
// bits.
_Static_assert(sizeof(wpc_frame_t) <= (3 + UINT8_MAX), "");
